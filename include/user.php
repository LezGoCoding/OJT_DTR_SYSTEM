...
class User{
	...
	function single_user($id=0){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where USERID = :id LIMIT 1", [':id' => $id]);
	...
	static function AuthenticateUser($username = "", $password = "") {
		global $mydb;
		$sql = "SELECT * FROM user WHERE `EMPID` = :username LIMIT 1";
		$params = [
			':username' => $username
		];
		...
		if ($row_count == 1) {
			...
			if (password_verify($password, $found_user->PASSWORD)) {
				$_SESSION['ACCOUNT_ID'] 	 	= $found_user->USERID;
				$_SESSION['ACCOUNT_NAME'] 		= $found_user->FULLNAME;
				$_SESSION['ACCOUNT_USERNAME']	= $found_user->EMPID;
	...
	public function update($id = 0) {
		...
		try {
			...
			$sql = "UPDATE " . self::$tbl_name . " SET " . join(", ", $attribute_pairs) . " WHERE `USERID` = :id";
	...
	public function delete($id = 0) {
		...
		try {
			...
			$sql = "DELETE FROM " . self::$tbl_name . " WHERE `USERID` = :id LIMIT 1";
